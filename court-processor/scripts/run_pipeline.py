...
async def run_pipeline(limit: int = 10, strict: bool = True, force: bool = False, unprocessed_only: bool = False, extract_pdfs: bool = False):
    """Run the pipeline with specified parameters"""
    print(f"\n{'='*80}")
    print(f"Running Pipeline: {limit} documents")
    print(f"  Options: strict={strict}, force_reprocess={force}, unprocessed_only={unprocessed_only}, extract_pdfs={extract_pdfs}")
    print(f"{'='*80}\n")
    ...
    try:
        ...
        results = await pipeline.process_batch(
            ...
            only_unprocessed=unprocessed_only,
        ...
        if results['success']:
            ...
            print(f"  Total citations extracted: {quality.get('total_citations_extracted', stats['citations_extracted'])}")
...
def main():
    ...
    parser.add_argument('--force', action='store_true',
                       help='Force reprocess documents even if unchanged')
    parser.add_argument('--unprocessed', action='store_true',
                       help='Only process documents not yet in opinions_unified')
    parser.add_argument('--extract-pdfs', action='store_true',
                       help='Extract content from PDFs when available')
