#!/usr/bin/env python3
"""
Eleven Stage Pipeline Implementation - Improved Version
Enhanced completeness scoring and better data extraction
"""
...
class ImprovedElevenStagePipeline:
    """
    Improved 11-stage document processing pipeline with better completeness
    """
    ...
    async def process_batch(self, 
        ...
        try:
            ...
            for idx, doc in enumerate(documents):
                ...
                # ==========================================
                # STAGE 2: Court Resolution Enhancement (IMPROVED)
                # ==========================================
                if idx == 0:  # Log stage header once
                    logger.info("\n" + "=" * 60)
                    logger.info("STAGE 2: Court Resolution Enhancement (Improved)")
                    logger.info("=" * 60)
                
                court_enhancement = self._enhance_court_info_improved(doc)
                ...
                # ==========================================
                # STAGE 4: Reporter Normalization (FIXED)
                # ==========================================
                if idx == 0:
                    logger.info("\n" + "=" * 60)
                    logger.info("STAGE 4: Reporter Normalization (Fixed)")
                    logger.info("=" * 60)
                
                normalized_reporters = self._normalize_reporters_fixed(citations['citations'])
                ...
                # ==========================================
                # STAGE 5: Judge Information Enhancement (IMPROVED)
                # ==========================================
                if idx == 0:
                    logger.info("\n" + "=" * 60)
                    logger.info("STAGE 5: Judge Information Enhancement (Improved)")
                    logger.info("=" * 60)
                
                judge_info = self._enhance_judge_info_improved(doc)
            ...
            # ==========================================
            # STAGE 11: Pipeline Verification (IMPROVED)
            # ==========================================
            logger.info("\n" + "=" * 60)
            logger.info("STAGE 11: Pipeline Verification (Improved)")
            logger.info("=" * 60)
            
            verification = self._verify_pipeline_results_improved(enhanced_documents)
            ...
            return {
                ...
                'improvements': {
                    'court_extraction_from_content': court_enhancement.get('extracted_from_content', False),
                    'judge_extraction_from_content': judge_info.get('extracted_from_content', False),
    ...
    def _enhance_court_info_improved(self, document: Dict[str, Any]) -> Dict[str, Any]:
        """Stage 2: Enhanced court resolution with content extraction"""
        ...
        # If no court info in metadata, try to extract from content
        if not court_hint:
            court_hint = self._extract_court_from_content(document.get('content', ''))
        ...
        try:
            ...
            if court_ids:
                ...
                return {
                    ...
                    'extracted_from_content': court_hint not in [
                        metadata.get('court_id', ''), 
                        metadata.get('court', ''), 
    ...
    def _extract_court_from_content(self, content: str) -> Optional[str]:
        """Extract court name from document content"""
        if not content:
            return None
        ...
        content_start = content[:1000].upper()
    ...
    def _normalize_reporters_fixed(self, citations: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Stage 4: Fixed reporter normalization with proper counting"""
        normalized = []
        ...
        for citation in citations:
            ...
            if reporter:
                ...
                if reporter in REPORTERS:
                    ...
                    # Also check if reporter matches a key case-insensitively
                    if not found:
    ...
    def _enhance_judge_info_improved(self, document: Dict[str, Any]) -> Dict[str, Any]:
        """Stage 5: Enhanced judge information extraction"""
        metadata = document.get('metadata', {})
        if not isinstance(metadata, dict):
            metadata = {}
        ...
        if not judge_name:
            judge_name = self._extract_judge_from_content(document.get('content', ''))
                ...
                for judge_info in judges_data:
                    ...
                    if judge_lower in person_name.lower() or person_name.lower() in judge_lower:
                        return {
                            ...
                            'source': 'judge-pics',
                            'extracted_from_content': judge_name not in [
    ...
    def _extract_judge_from_content(self, content: str) -> Optional[str]:
    ...
    def _assemble_metadata(self, document: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'pipeline_version': '11-stage-improved-v1',
    ...
    def _verify_pipeline_results_improved(self, documents: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Stage 11: Improved pipeline verification with correct counting"""
        verification = {
        ...
        for doc in documents:
            ...
            # Normalized reporters (FIXED: check if any reporters were normalized)
...
async def main():
    """Run the improved eleven stage pipeline"""
    pipeline = ImprovedElevenStagePipeline()
    
    print("\n" + "🚀 " * 20)
    print("ELEVEN STAGE PIPELINE - IMPROVED IMPLEMENTATION")
    print("🚀 " * 20 + "\n")
    ...
    if results['success']:
        ...
        print(f"\n🔧 IMPROVEMENTS:")
        for key, value in results.get('improvements', {}).items():
            print(f"   {key}: {value}")
    else:
