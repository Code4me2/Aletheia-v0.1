...
const { encrypt, needsEncryption, ENCRYPTED_FIELDS } = require('../src/lib/crypto');
...
async function reencryptModel(model, encryptedFields) {
  console.log(`\n🔍 Processing ${model}...`);
...
async function main() {
  ...
  for (const [model, fields] of Object.entries(ENCRYPTED_FIELDS)) {
    ...
    if (prisma[model]) {
      ...
      console.log(`\n⚠️  Skipping ${model} - model not found in Prisma`);
    }
  }
